...
public class TreeImpl<E extends Comparable<? super E>> implements Tree<E> {
    ...
    private static final int LEVEL_INFINITY = -1;
    private Node<E> root;
    private int size;
    private int maxLevel = LEVEL_INFINITY;
    ...
    public boolean add(E value) {
        ...
        if (maxLevel != LEVEL_INFINITY && maxLevel < height(root)) {
            remove(value);
            return false;
        } else {
            size++;
    ...
    public boolean isCanAdd() {
        if (maxLevel == LEVEL_INFINITY) {
            return true;
        }

        if (Math.pow(2, maxLevel) - 1 <= size) {
            return false;
        }
